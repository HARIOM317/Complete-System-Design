...
public:
    ...
    // Called by Channel: prints notifications message
...
int main()
{
    ...
    Channel *channel = new Channel("HSR");

    Subscriber *subs1 = new Subscriber("Anand", channel);
    Subscriber *subs2 = new Subscriber("Varun", channel);

    // Anand and Varun subscribe to "HSR"
    channel->subscribe(subs1);
    channel->subscribe(subs2);

    // Upload a video: both Anand and Varun are notified
    channel->uploadVideo("Observer Pattern Tutorial");

    // Varun unsubscribes; Anand remains subscribed
    channel->unsubscribe(subs1);

    // Upload another video: only Anand is notified
