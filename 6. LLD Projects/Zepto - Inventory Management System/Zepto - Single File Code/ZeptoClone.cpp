...
public:
    ...
    void replenish(InventoryManager *manger, map<int, int> itemsToReplenish) override
    {
...
public:
    DarkStore(string n, double x_coord, double y_coord)
    {
        ...
        // We could have made another factory called InventoryStoreFactory to get DbInventoryStore by enum and hence make it loosely coupled.
    ...
    InventoryManager *getInventoryManger()
    {
...
//////////////////////////////////////////////
class DarkStoreManger
{
private:
    vector<DarkStore *> *darkStores;
    static DarkStoreManger *instance;
...
public:
    static DarkStoreManger *getInstance()
    {
        if (instance == nullptr)
        {
            instance = new DarkStoreManger();
        }
        return instance;
    }
...
DarkStoreManger *DarkStoreManger::instance = nullptr;
...
// __________ Singleton __________
class OrderManager
{
private:
    vector<Order *> *orders;
...
public:
    ...
    void placeOrder(User *user, Cart *cart)
    {
        ...
        double nextDist = 5.0;
        vector<DarkStore *> nearbyDarkStores = DarkStoreManger::getInstance()->getNearbyDarkStores(user->x, user->y, nextDist);
        ...
        else
        {
            ...
            for (DarkStore *store : nearbyDarkStores)
            {
                ...
                // After iterating all SKUs in allItems, we erase any fully‐satisfied SKUs from the allItems map.
                for (int sku : toErase)
                    allItems.erase(sku);

                // If at least one SKU was taken from this store, we assign a new DeliveryPartner.
                if (assigned)
                {
...
public:
    static void showAllItems(User *user)
    {
        ...
        DarkStoreManger *dsManager = DarkStoreManger::getInstance();
    ...
    {
        auto dsManager = DarkStoreManger::getInstance();
...
int main()
{
    ...
    // 6) Cleanup
    delete user;
    delete DarkStoreManger::getInstance(); // deletes all DarkStores and their inventoryManagers
