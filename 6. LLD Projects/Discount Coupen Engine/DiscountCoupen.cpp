...
// Percentage With Cap Discount Strategy
...
DiscountStrategyManager *DiscountStrategyManager::instance = nullptr; // Initialize static member
...
public:
    BulkPurchaseDiscount(double thr, double off)
    {
        threshold = thr;
        flatOff = off;
        strategy = DiscountStrategyManager::getInstance()->getStrategy(StrategyType::FLAT, flatOff);
    }
...
public:
    BankingCoupon(const string &b, double ms, double percent, double offCap)
    {
        ...
        this->offCap = percent;
        strategy = DiscountStrategyManager::getInstance()->getStrategy(StrategyType::PERCENT_WITH_CAP, percent, offCap);
    }
...
CouponManager *CouponManager::instance = nullptr; // Initialize static instance pointer
...
int main()
{
    ...
    mgr->registerCoupon(new BankingCoupon("SBI", 2000, 50, 500));
